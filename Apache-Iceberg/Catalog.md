...
Каталог не просто хранит ссылку на метаданные — он гарантирует атомарность операций. Когда обновляем таблицу (добавляем данные, меняем схему), каталог атомарно переключает указатель со старого файла метаданных на новый. Это значит, что переключение происходит целиком или не происходит вообще — читатели никогда не увидят «полуобновлённое» состояние таблицы.

Именно атомарность даёт нам консистентные чтения (все читатели видят одинаковую версию данных) и безопасные параллельные записи (два писателя не испортят данные друг другу).
