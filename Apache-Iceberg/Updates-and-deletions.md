...
| **Критерий**                       | **Copy-on-Write (CoW)**                     | **Merge-on-Read (MoR)**                        |
| ---------------------------------- | ------------------------------------------- | ---------------------------------------------- |
| **Производительность записи**      | Низкая — требуется перезапись целых файлов  | Высокая — записываются только изменения        |
| **Производительность чтения**      | Высокая — чтение готовых файлов             | Ниже — требуется объединение данных при чтении  (слияние на лету) |
| **Накладные расходы на хранилище** | Выше — могут появляться дубликаты данных    | Ниже — хранятся изменения и delete-файлы (хранятся только дельты)        |
...
В следующем уроке мы изучим Типы данных и эволюцию схемы — как Iceberg позволяет безопасно менять структуру таблиц без перезаписи данных.
