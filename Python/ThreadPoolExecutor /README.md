...
- [Потоки и процессы](#Потоки-и-процессы)
    - [Потоки и процессы в Python](#Потоки-и-процессы-в-Python)
    - [threading.Thread (ручной поток) потоки внутри одного процесса](#threadingthread-ручной-поток-потоки-внутри-одного-процесса)
    - [threading.Lock (защита счетчика)](#threadingLock-защита-счетчика)
    - [Обмен данными между потоками: queue.Queue](#Обмен-данными-между-потоками-queueQueue)
    - [threading.local (свои данные для каждого потока)](#threadinglocal-свои-данные-для-каждого-потока)
    ...
    - [Когда что брать](#Когда-что-брать)
- [Асинхронное програмирование](#Асинхронное-програмирование)
    - [Основы asyncio в Python](#Основы-asyncio-в-Python)
    - [Цикл событий и кооперативная модель](#Цикл-событий-и-кооперативная-модель)
    - [async и await](#async-и-await)
    - [asyncio.run: точка входа](#asynciorun-точка-входа)
    - [Последовательно vs конкурентно](#Последовательно-vs-конкурентно)
    - [Tasks: запуск корутин «в фоне»](#Tasks-запуск-корутин-в-фоне)
- [Продвинутое асинхронное програмирование](#Продвинутое-асинхронное-програмирование)
    - [Продвинутый asyncio в Python](#Продвинутый-asyncio-в-Python)
    - [asyncio.Queue: обмен данными между корутинами](#asyncioQueue-обмен-данными-между-корутинами)
    - [asyncio.Lock: защита общего состояния](#asyncioLock-защита-общего-состояния)
    - [Блокирующий код в asyncio: run_in_executor](#Блокирующий-код-в-asyncio-run_in_executor)
    - [async-итерация и контекстные менеджеры](#async-итерация-и-контекстные-менеджеры)
    - [Сравнение трёх подходов](#Сравнение-трёх-подходов)
    - [Правило выбора:](#Правило-выбора)
    - [Несколько подводных камней](#Несколько-подводных-камней)
