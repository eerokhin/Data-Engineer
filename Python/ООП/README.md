...
2. [Объект (экземпляр класса)](#2-объект-экземпляр-класса-instance)
3. [Атрибуты](#3-атрибуты-attributes)
4. [Короткое определение](#4-короткое-определение)
5. [Метод](#5-метод-method)
6. [Что такое self, для чего нужен, как и где используется](#6-что-такое-self-для-чего-нужен-как-и-где-используется)
...
## 6. Что такое Self, для чего нужен, как и где используется?
...
Параметр `self` — это ссылка на конкретный объект (экземпляр класса), который создаётся в данный момент. Через него мы сохраняем данные внутрь объекта.

`self` нужен, чтобы: хранить данные внутри конкретного объекта, обращаться к его атрибутам, вызывать другие методы этого же объекта.

`self` используется внутри методов класса (например, в `__init__` и обычных методах), чтобы Python понимал, с каким именно объектом сейчас идёт работа.
